...
*** DONE Il grafo degli autori deve essere costruito usando la classe Attore nel seguente modo:
...
6. le istruzioni per la compilazione del programma java devono essere inserite nel makefile in modo che la compilazione avvenga scrivendo semplicemente make sulla linea di comando; successivamente il programma verrà testato con il comando java CreaGrafo attori.tsv titoli.tsv (con opportuni file .tsv). Se questa procedura non funziona (per qualsiasi motivo il programma non viene eseguito correttamente) il progetto sarà considerato insufficiente senza ulteriori indagini.
      ...
      Apro il file title.principals.tsv per l'analisi dei cast: usiamo una HashMap per creare i cast di ogni film (<film,cast>), dopodiché in base ai cast aggiorniamo i campi coprot e ncollab (nuovo campo aggiunto= #coprot) per prepararsi alla stamppa su grafo.txt
      NOTA: per l'ordinamento dei codici degli attori abbiamo a disposizione la vecchia struttura dati della prima parte, mentre per l'ord dei coprot si usa TreeSet che aggiorna l'ordinamento mano mano.
...
- TreeSet buffer file (laptop): 64KB-> 1m49,676s / 1m47,423s, 8KB-> 2m1,410s / 1m57,961s
- HashSet buffer file (laptop): 64KB-> 1m16,550s / 1m18,599s, 8KB-> 1m18,197s / 1m17,869s
        L'approccio migliore è HashSet invece che TreeSet, i coprot andranno letti una volta sola quindi il costo è uguale ma con prestazioni migliori!
        INOLTRE: facendo confronti entrambe le versioni necessitano di una quantità di RAM tra 4 e 5 GB-> la differenza è molto bassa, tanto vale usare un HashSet!


* IDEA cammini.c

La seconda parte del progetto consiste in un programma C che legge i dati del grafo degli attori ed effettua il calcolo di cammini minimi. All'intero del programma C i nodi del grafo devono essere rappresentati dal tipo attore così definito:
...
+ Se il grafo ha tot attori esso deve essere rappresentato da un array attori contenente tot oggetti di tipo attore ordinati per codice crescente, in altre parole deve essere attori[i].codice < attori[i+1].codice per i=0,...,tot-2.
...
1. per prima cosa si legge il file nomi.txt e si crea l'array attori inizializzando solamente i campi codice, nome e anno di ogni attore
        Si crea array di dimensione dinamica tramite malloc e realloc. L'array dovrà essere di tipo "attore*".
        Per farlo si dovrà creare una funzione di parsing delle righe come si era fatto anche in java (tokenize)
...
2. successivamente si deve leggere il file grafo.txt utilizzando uno schema produttori consumatori. Il thread principale svolge il ruolo di produttore e legge le linee del file una alla volta con getline(3) e copia il puntatore alla linea letta su un buffer condiviso con i consumatori (il cui numero è dato dal parametro di input numconsumatori). I consumatori leggono le linee che vengono loro passate e utilizzano i dati in essa contenuti per inizializzare i campi numcop e cop di ogni attore. Dopo aver completato la lettura del file grafo.txt questi thread consumatori possono terminare
       Definire tokenize() per il parsing di una linea
       cons_body per i consumatori e daticons
       prod_body per il prod e datiprod
...
** HOLD Thread per il calcolo del cammino minimo

- Questi thread ricevono come input il grafo (in pratica l'array attori) e il codice di due attori a e b e devono effettuare la ricerca di un cammino minimo da a a b all'interno del grafo utilizzando l'algoritmo BFS (in pratica è l'algoritmo di Dijkstra ottimizzato per il caso in cui gli archi hanno costo unitario).
        L'algoritmo utilizza una coda FIFO che potete implementare come ritenete opportuno (ma deve essere in grado di gestire un numero arbitrariamente grande di elementi, quindi un array di dimensione fissata non è accettabile). L'algoritmo deve anche tenere traccia dei nodi che sono già stati inseriti nella coda (variabile explored nello pseudocodice), a tale scopo è obbligatorio utilizzare un ABR (albero binario di ricerca). Questi thread devono essere resi detached (cioè non richiedere che venga fatta la join) con una opportuna chiamata alla funzione pthread_detach(3)
               per l'impl serve una fifo per i nodi da esplorare (aggiunti mano a mano che si esplorano i nodi-> un idea è di usare una linked list per gli einserimenti e estrazioni efficienti, senza usare un altro array dinamico), struttura per gli attori esplorati i cui nodi dovranno contenere anche predecessore nel cammino (per ricorstruirlo) e distanza dalla sorgente -> ABR

1. Ogni thread deve scrivere il proprio output sul file di nome a.b dove a e b sono i codici ricevuti in input. Ad esempio se il thread deve calcolare il cammino minimo tra i nodi 1000 e 2000, l'output deve essere scritto sul file 1000.2000 e deve consistere di uno dei seguenti messaggi:
    codice 1000 non valido; se ad esempio il codice 1000 non corrisponde ad un attore nel grafo
...
2. Oltre a questo file di output, ogni thread deve stampare su stdout una riga di testo che descrive i codici ricevuti input, l'esito della computazione, e l'indicazione del tempo di calcolo in secondi; ad esempio:
...
* PROJ Consegna del progetto:
- La consegna deve avvenire esclusivamente mediante GitHub. Se non lo avete già createvi un account e create un repository privato dedicato a questo progetto. Aggiungete come collaboratore al repository l’utente Laboratorio2B in modo che i docenti abbiano accesso al codice.
- IMPORTANTE: la consegna effettiva del progetto per un dato appello consiste nello scrivere l'url per la clonazione del vostro progetto nell'apposito contenitore consegna su moodle. L'url deve essere della forma git@github.com:user/progetto.git (non deve quindi iniziare per https). Dopo la data di consegna non dovete fare altri commit al progetto. Ricordatevi che almeno cinque giorni prima della data di consegna dovete iscrivervi all'appello su esami.unipi.it.

Nel repository dovete mettere solamente i file veramente essenziali per la compilazione del progetto: quindi non i file .o gli eseguibili, i file di configurazione o i file di input che vi ho fornito io.
All'interno del repository deve essere presente un file README.md contente la descrizione dei seguenti punti:

    - il dettaglio di come avviene il parsing delle righe del file di testo name.basics.tsv nel programma Java (aggiornare dopo la modifica)
    + il dettaglio dell'implementazione della coda FIFO nell'algoritmo BFS, e quali informazioni vengono memorizzate in ogni elemento della coda
    + il dettaglio di come vengono ricostruiti i nodi intermedi del cammino minimo in cammini.c
    - il dettaglio di come il thread gestore di segnali comunica al programma principale di interrompere l'elaborazione

NOTA: risitemare tutti e 4 i punti

* File messi a disposizione:
- I file name.basics.tsv (842Mb), title.principals.tsv (3.9 Gb), sono disponibili sulla macchina laboratorio2.di.unipi.it. Per fare i test del vostro programma su questa macchina NON copia sulla vostra home un'altra istanza di questi file (altrimenti esauriamo subito lo spazio disco), ma createvi dei link simbolici in una qualsiasi vostra directory scrivendo:
        ln -s /home/local/ADUNIPI/a048846/2425/name.basics.tsv
        ln -s /home/local/ADUNIPI/a048846/2425/title.principals.tsv
        ln -s /home/local/ADUNIPI/a048846/2425/title.1m.tsv

(il file title.1m.tsv è menzionato qui sotto) Potete anche copiarvi questi file sulla vostra macchina locale (usando ad esempio scp).
- Su moodle trovate anche un archivio files progetto contenente:
    i file nomi.txt e grafo1m.txt contenenti il grafo prodotto da CreaGrafo utilizzando al posto di title.principals.tsv il file title.1m.tsv contenente solamente il primo milione di titoli, ottenuto con il comando head -n 1000000 title.principals.tsv > title.1m.tsv; suggerisco di usare questo grafo ridotto per i primi esperimenti.
    il file 100grafo.txt contenente le prime 100 righe del file grafo.txt ottenuto utilizzando tutti i titoli in title.principals.tsv
    il programma cammini.py menzionato sopra

* Test peogetto obbligatorio:
Prima della consegna dovete verificare che il progetto soddisfi ai requisiti base utilizzando i file controllaProgetto.py e miniN.tsv. Per il controllo lo script controllaProgetto.py oltre a miniN.tsv ha bisogno di title.1m.tsv e, per il progetto completo, title.basics.tsv; se necessario create un link simbolico a questi file nella stessa directory di controllaProgetto.py.

A questo punto eseguite controllaProgetto.py (con l'opzione -r se dovete consegnare il progetto ridotto): lo script provvederà a compilare ed a eseguire le varie componenti del progetto. Se al termine compare il messaggio

        ### Test concluso senza rilevare errori
il test è superato e potete consegnare. In caso contrario viene stampato il messaggio

        Il progetto non può essere consegnato in questa forma:
        correggi gli errori e ri-esegui il test
insieme ad una spiegazione del problema (a volte manca semplicemente un file). Non consegnate fino a quando questo test minimale non viene superato altrimenti il progetto sarà valutato non sufficiente indipendentemente dal contenuto.

Naturalmente il controllo va effettuato sui file che avete consegnato: il mio consiglio è di fare una clone del progetto su una nuova directory con il comando git clone ilmioprogetto.git nuovadir e di effettuare il test dentro la directory nuovadir


* TODO IDEE:
 - ripulire xerrori.* e sistemare utils.*
 - provare con array dinamico invece di ll
 + finire il README
 + studiare cammii.py
 + verificare i controlli sulle maloc e realloc
